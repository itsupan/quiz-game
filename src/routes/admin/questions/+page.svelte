...
<div class="head">
	<div>
		<h1>Questions</h1>
		<p class="lede">{data.total} in the bank.</p>
	</div>
	<a class="primary" href={resolve('/admin/questions/new')}>New question</a>
</div>
...
<!-- A GET form, so a filtered list is a shareable URL and works without JavaScript. -->
...
{#if data.items.length === 0}
	<p class="empty" data-testid="empty">
		No questions match these filters. <a href={resolve('/admin/questions/new')}>Add one</a>.
	</p>
{:else}
	<table>
		<thead>
		...
		<tbody>
			{#each data.items as question (question.publicId)}
				<tr>
					<td>
						<a href={resolve('/admin/questions/[publicId]', { publicId: question.publicId })}>
							{question.stem}
						</a>
					</td>
					...
					<td>
						...
						{#if !question.hasAnswerKey}
							...
							<span class="flag">no answer key</span>
						{/if}
					</td>
					<td><StatusBadge status={question.status} /></td>
					<td class="when">{dateFormat.format(question.updatedAt)}</td>
					<td class="actions">
