...
>
	<div class="flex h-16 w-full items-stretch justify-around">
		{#each items as item, i (item.href)}
			{@const current = isCurrent(item.href, i)}
			<NavLink
				href={item.href}
				{current}
				class="group relative flex flex-1 flex-col items-center justify-center px-1 py-1.5 text-center transition-colors select-none {current
					? 'text-brand-red'
			...
			>
				...
				<i
					class="fi fi-rs-{resolveIcon(
						item
					)} text-xl leading-none transition-transform group-active:scale-90"
					aria-hidden="true"
				></i>

				<span
					class="mt-1 max-w-full truncate text-[10px] tracking-wider uppercase transition-all {current
						? 'font-black text-brand-red'
						: 'font-bold text-stone-600 group-hover:text-ink'}"
				>
					{item.label}
				</span>
			</NavLink>
