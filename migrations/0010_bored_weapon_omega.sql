...
CREATE TABLE `__new_attempts` (
	...
	CONSTRAINT "attempts_bonus_xp_check" CHECK("__new_attempts"."bonus_xp_awarded" >= 0)
);
...
CREATE INDEX `attempts_guest_purge_idx` ON `attempts` (`status`,`started_at`) WHERE "attempts"."user_id" is null;